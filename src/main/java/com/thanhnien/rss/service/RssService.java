...
public class RssService {
        ...
        public RssFeed fetchRss(String rssUrl) {
                return fetchRss(rssUrl, -1);
        }

        @Cacheable(value = "rssFeed", key = "{#rssUrl, #limit}", unless = "#result == null || #result.articles.isEmpty()")
        public RssFeed fetchRss(String rssUrl, int limit) {
                int maxRetries = 3;
                int retryCount = 0;
                ...
                while (retryCount < maxRetries) {
                        ...
                        try {
                                logger.info("Fetching RSS from: {} (limit: {}, attempt {})", rssUrl, limit,
                                                retryCount + 1);
        ...
        public RssFeed getHomeArticles() {
                return self.fetchRss(BASE_RSS_URL + "home.rss", 5);
        }
        ...
        public RssFeed getArticlesByCategory(String slug) {
                return getArticlesByCategory(slug, -1);
        }

        public RssFeed getArticlesByCategory(String slug, int limit) {
                Category category = findCategoryBySlug(slug);
                if (category != null) {
                        RssFeed feed = self.fetchRss(category.getRssUrl(), limit);
        ...
        public HomePageData getHomePageData() {
                ...
                CompletableFuture<List<Article>> featuredFuture = CompletableFuture.supplyAsync(() -> {
                        RssFeed feed = self.getHomeArticles(); // Already limited to 5
                ...
                List<CompletableFuture<CategorySection>> categoryFutures = this.categories.stream()
                                ...
                                .map(cat -> CompletableFuture.supplyAsync(() -> {
                                        RssFeed feed = self.getArticlesByCategory(cat.getSlug(), 5);
